    ...
    computed: {
        cantidad_a_regresar: function () {
            ...
            if (Number(this.form.formaPago.value) == 1) {
                /**efectivo */
                total =
                    parseFloat(this.form.pago_con_cantidad) -
            ...
            if (this.form.pagos_a_cubrir.length > 0) {
                this.form.pagos_a_cubrir.forEach((element) => {
                    if (element.status_pago != 2) {
                        /**diferente de pagado */
                        maximo +=
                            Number(element.monto_programado) -
                            Number(element.total_cubierto);
                    }
                });
            }
    ...
    methods: {
        async get_cobradores() {
            try {
                ...
                this.$vs.notify({
                    title: "Error",
                    text: "Ha ocurrido un error al tratar de cargar el catálogo de cobradores, por favor reintente.",
                    iconPack: "feather",
        ...
        async get_monedas_sat() {
            try {
                ...
                this.$vs.notify({
                    title: "Error",
                    text: "Ha ocurrido un error al tratar de cargar el catálogo de moneda de pago, por favor reintente.",
                    iconPack: "feather",
                    icon: "icon-alert-circle",
                    color: "danger",
                    position: "bottom-right",
        ...
        async get_formas_pago_sat() {
            try {
                ...
                this.$vs.notify({
                    title: "Error",
                    text: "Ha ocurrido un error al tratar de cargar el catálogo de formas de pago, por favor reintente.",
                    iconPack: "feather",
                    icon: "icon-alert-circle",
                    color: "danger",
                    position: "bottom-right",
        ...
        checarPagos(event) {
            ...
            if (event.target.checked == true) {
                ...
                if (this.form.datos_operacion.pagos_programados) {
                    this.form.datos_operacion.pagos_programados.forEach(
                        (pago) => {
                            if (pago.status_pago != 2) {
                                /**no pagados */
                                this.$refs["pago_seleccionado"][
                                    index_pago
        ...
        async calcular_adeudo() {
            ...
            try {
                ...
                this.$nextTick(() => {
                    let index_pago = 0;
                    if (
                        this.form.datos_operacion.pagos_programados.length > 0
                    ) {
                        this.form.datos_operacion.pagos_programados.forEach(
                            (pago) => {
                                if (pago.status_pago != 2) {
