<template>
    <div>
        <vs-popup :class="['forms-popup', popupSize, z_index]" :fullscreen="false" close="cancelar" :title="popupTitle"
            ...
            <Password :z_index="'z-index61k'" v-if="openPassword" :show="openPassword" :callback-on-success="callback"
...
export default {
    ...
    computed: {
        ...
        isReadOnly() {
            return this.tipo === "consultar" || this.tipo === "cancelar";
        },
        verEnviarEmailChk() {
            if (this.tipo === "cancelar") {
                return this.formData.email_seguimiento ? true : false;
            } else return true;
        },
        popupSize() {
            let size = "popup-70";
            if (
                (this.fueProgramado && this.tipo === "cancelar") ||
                this.fueProgramado
            ) {
                size = "popup-100";
            }
    ...
    methods: {
        ...
        resultado_datos_cliente(response, email = null) {
            if (response) {
                ...
                this.$error("🚀 ~ resultado_datos_cliente ~ error:", response);
                this.cancelar();
            }
        },
        checkReady() {
        ...
        async _loadSeguimientoDatos() {
            ...
            const params = {
                ...
                programado_b:
                    this.tipo === "atender_seguimiento_programado" ? 1 : 0,
                status: 1,
            ...
            try {
                ...
                if (
                    ...
                    this.$nextTick(() => {
                        ...
                        this.formData.email_seguimiento =
                            result[0].email_seguimiento;
                        ...
                        let fecha_contacto =
                            result[0].fecha_seguimiento.split("-");
                        let hora_contacto =
                            result[0].hora_seguimiento.split(":");
                        //yyyy-mm-dd hh:mm
