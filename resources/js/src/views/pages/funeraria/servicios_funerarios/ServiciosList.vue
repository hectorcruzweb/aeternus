...
export default {
    ...
    methods: {
        OpenFormSeguimientos(venta) {
            ...
            if (venta.operacion) {
                this.filtersSeguimientos.cliente_id =
                    venta.operacion.clientes_id;
        ...
        async get_data(page, evento = "") {
            ...
            try {
                let res = await funeraria.get_solicitudes_servicios(
                    this.serverOptions,
                    true
                );
                if (res.data.data) {
