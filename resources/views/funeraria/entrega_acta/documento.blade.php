...
<body>
    ...
    <h1 class="center mt-5 size-22px">FORMATO DE ENTREGA DE ACTA DE DEFUNCIÓN</h1>
    ...
    <div class="contenido w-100 mt-5">
        ...
        <p class="justificar line-lg size-18px">
            ...
            levantada el día
            <span class="bold">
                @if ($datos['fechahora_acta'] == null)
                    N/A
                @else
                    {{ fecha_only($datos['fechahora_acta']) }}
                @endif
            </span>,
            correspondiente a el/la ( @if (!isset($datos['titulo']['titulo']))
                N/A
            @else
                {{ $datos['titulo']['titulo'] }}
